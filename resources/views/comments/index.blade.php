<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-4 flex justify-end">
                        <a href="{{ route('comments.create') }}" class="inline-flex items-center bg-blue-600 text-white py-2 px-6 rounded-lg shadow-md hover:bg-blue-700 focus:outline-none transition ease-in-out duration-300 transform hover:scale-105">
                            <i class="mr-2 text-sm"></i> {{ __('Agregar comentario') }}
                        </a>
                    </div>

                    <!-- Tabla de Comentarios -->
                    <div class="overflow-x-auto bg-gray-50 shadow-md rounded-lg">
                        <table id="comments-table" class="min-w-full text-sm text-left text-gray-600">
                            ...
                            @foreach($comments as $comment)
                                <tr class="bg-white border-b hover:bg-gray-100 transition-colors duration-300">
                                    ...
                                    <td class="text-sm text-center">
                                        <!-- Botón Ver -->
                                        <a href="{{ route('comments.show', $comment->id) }}" class="text-blue-600 hover:text-blue-800 transition-colors duration-300 text-xl">
                                            <i class="fas fa-eye"></i>
                                        </a>

                                        <!-- Botón Editar -->
                                        <a href="{{ route('comments.edit', $comment->id) }}" class="text-yellow-500 hover:text-yellow-700 transition-colors duration-300 text-xl">
                                            <i class="fas fa-edit"></i>
                                        </a>

                                        <!-- Botón Eliminar -->
                                        <button class="text-red-600 hover:text-red-800 transition-colors duration-300 delete-btn text-xl" data-comment-id="{{ $comment->id }}">
    ...
    <div id="confirm-delete-modal" class="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 hidden">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            <h3 class="text-lg font-semibold text-gray-700">{{ __('¿Estás seguro de que deseas eliminar este comentario?') }}</h3>
    ...
    <script>
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                deleteForm.action = `/comments/${commentId}`;  // Configura la acción del formulario para eliminar el comentario
